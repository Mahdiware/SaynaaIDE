...
public class FileProvider extends ContentProvider {
  private static final String[] DEFAULT_PROJECTION = {OpenableColumns.DISPLAY_NAME, OpenableColumns.SIZE};
  ...
  private static final Map<String, PathStrategy> STRATEGIES = new HashMap<>();
  ...
  public void attachInfo(Context context, ProviderInfo info) {
    ...
    if (info.exported) {
      throw new SecurityException("FileProvider must not be exported");
    }

    if (!info.grantUriPermissions) {
      throw new SecurityException("FileProvider must grant URI permissions");
    }

    if (TextUtils.isEmpty(info.authority)) {
      throw new SecurityException("FileProvider authority must not be empty");
    }
  ...
  public Uri insert(Uri uri, ContentValues values) {
    throw new UnsupportedOperationException("FileProvider does not support insert");
  }

  @Override
  public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
    throw new UnsupportedOperationException("FileProvider does not support update");
  }
  ...
  private static PathStrategy getPathStrategy(Context context, String authority) {
    synchronized (CACHE_LOCK) {
      ...
      try {
        parsed = parsePathStrategy(context, authority);
      } catch (IOException e) {
        throw new IllegalArgumentException("Failed to parse " + META_DATA_FILE_PROVIDER_PATHS, e);
  ...
  private static PathStrategy parsePathStrategy(Context context, String authority)
    ...
    SimplePathStrategy strategy = new SimplePathStrategy(authority);
    ...
    if (info == null) {
      throw new IllegalArgumentException("Couldn't find provider for authority: " + authority);
    }
    ...
    try {
      ...
      while ((event = parser.next()) != XmlPullParser.END_DOCUMENT) {
        ...
        case TAG_EXTERNAL_FILES_PATH:
          target = firstNonNull(getExternalFilesDirs(context, null));
          break;

        case TAG_EXTERNAL_CACHE_PATH:
          target = firstNonNull(getExternalCacheDirs(context));
          break;

        case TAG_EXTERNAL_MEDIA_PATH:
          if (android.os.Build.VERSION.SDK_INT >= 21) {
            target = firstNonNull(context.getExternalMediaDirs());
          }
          break;
        }
  ...
  static final class SimplePathStrategy implements PathStrategy {
    ...
    void addRoot(String name, File root) {
      if (TextUtils.isEmpty(name)) {
        throw new IllegalArgumentException("Root name must not be empty");
      }
    ...
    public Uri getUriForFile(File file) {
      ...
      for (Map.Entry<String, File> entry : roots.entrySet()) {
        ...
        if (bestRoot == null || rootPath.length() > bestRoot.getValue().getPath().length()) {
          bestRoot = entry;
        }
      }

      if (bestRoot == null) {
        throw new IllegalArgumentException("Failed to find configured root that "
                                           + "contains " + path);
      }

      String rootPath = bestRoot.getValue().getPath();
      ...
      String encodedPath = Uri.encode(bestRoot.getKey()) + "/" + Uri.encode(relativePath, "/");
    ...
    public File getFileForUri(Uri uri) {
      ...
      if (!authority.equals(uri.getAuthority())) {
        throw new SecurityException("URI authority does not match provider");
      }

      String encodedPath = uri.getEncodedPath();

      if (TextUtils.isEmpty(encodedPath) || encodedPath.charAt(0) != '/') {
        throw new IllegalArgumentException("Invalid URI path: " + uri);
      }

      int splitIndex = encodedPath.indexOf('/', 1);

      if (splitIndex < 0) {
        throw new IllegalArgumentException("Invalid URI path: " + uri);
      }

      String rootName = Uri.decode(encodedPath.substring(1, splitIndex));

      String relativePath = Uri.decode(encodedPath.substring(splitIndex + 1));
    ...
    private static boolean isWithinRoot(String filePath, String rootPath) {
