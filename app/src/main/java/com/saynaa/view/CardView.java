...
public class CardView extends FrameLayout {
  ...
  private void initialize(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    if (attrs != null) {
      ...
      if (attrs.getAttributeResourceValue("http://schemas.android.com/apk/res/android", "background", 0) != 0) {
        // Card background is deliberately controlled by the Card API.
      }

      int minW = attrs.getAttributeResourceValue("http://schemas.android.com/apk/res/android", "minWidth", 0);

      int minH = attrs.getAttributeResourceValue("http://schemas.android.com/apk/res/android", "minHeight", 0);

      if (minW != 0 && context.getTheme().resolveAttribute(minW, value, true)) {
        userMinWidth = value.data;
      }

      if (minH != 0 && context.getTheme().resolveAttribute(minH, value, true)) {
  ...
  public void setCompatShadowEnabled(boolean enabled) {
    ...
    if (cardDrawable != null) {
      cardDrawable.setShadowEnabled(enabled && (softwareShadowForced || Build.VERSION.SDK_INT < 21));
    }
  ...
  public int getCardMinimumWidth() {
    int drawableWidth = cardDrawable == null ? 0 : (int) Math.ceil(cardDrawable.getMinWidth());

    return Math.max(userMinWidth, drawableWidth);
  }

  public int getCardMinimumHeight() {
    int drawableHeight = cardDrawable == null ? 0 : (int) Math.ceil(cardDrawable.getMinHeight());
  ...
  private void rebuildDrawable() {
    boolean software = softwareShadowForced || Build.VERSION.SDK_INT < 21;

    cardDrawable = new CardDrawable(backgroundColor, radius, elevation, maxElevation,
        preventCornerOverlap, software && compatShadowEnabled);
  ...
  private void updateCardPadding() {
    ...
    boolean nativeElevation = Build.VERSION.SDK_INT >= 21 && !softwareShadowForced;
    ...
    super.setPadding(contentPadding.left + shadowPadding.left, contentPadding.top + shadowPadding.top,
        contentPadding.right + shadowPadding.right, contentPadding.bottom + shadowPadding.bottom);
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    int requiredWidth = Math.max(measuredWidth, minWidth);

    int requiredHeight = Math.max(measuredHeight, minHeight);

    if (requiredWidth != measuredWidth || requiredHeight != measuredHeight) {
      setMeasuredDimension(resolveSize(requiredWidth, widthMeasureSpec),
  ...
  protected Parcelable onSaveInstanceState() {
    ...
    state.backgroundColor = backgroundColor == null ? Color.TRANSPARENT : backgroundColor.getDefaultColor();
  ...
  protected void onRestoreInstanceState(Parcelable state) {
    ...
    setContentPadding(saved.contentLeft, saved.contentTop, saved.contentRight, saved.contentBottom);
  }
  ...
  private static final class CardDrawable extends Drawable {
    ...
    private final Paint cornerShadowPaint = new Paint(Paint.ANTI_ALIAS_FLAG | Paint.DITHER_FLAG);
    ...
    CardDrawable(ColorStateList color, float radius, float elevation, float maxElevation,
        boolean preventCorners, boolean shadowEnabled) {
      this.color = color == null ? ColorStateList.valueOf(Color.TRANSPARENT) : color;
    ...
    public void draw(Canvas canvas) {
      ...
      int color = this.color.getColorForState(getState(), this.color.getDefaultColor());

      // Completely transparent card with no shadow.
      if (color == Color.TRANSPARENT && (!shadowEnabled || shadowSize <= 0f)) {
        return;
      }
      ...
      ColorFilter filter = colorFilter != null ? colorFilter : tintFilter;
    ...
    private void drawShadow(Canvas canvas) {
      ...
      canvas.translate(left + radius, top + radius);

      canvas.drawPath(cornerPath, cornerShadowPaint);

      canvas.translate(right - left - radius * 2, 0);

      canvas.rotate(90);
      canvas.drawPath(cornerPath, cornerShadowPaint);

      canvas.translate(bottom - top - radius * 2, 0);
      ...
      if (innerRight > innerLeft) {
        LinearGradient horizontal = new LinearGradient(innerLeft, top, innerLeft, top + shadowY,
            new int[] {0x00000000, 0x55000000, 0x00000000}, new float[] {0f, 0.5f, 1f},
            Shader.TileMode.CLAMP);

        shadowPaint.setShader(horizontal);

        canvas.drawRect(innerLeft, top - shadowY, innerRight, top, shadowPaint);

        canvas.drawRect(innerLeft, bottom, innerRight, bottom + shadowY, shadowPaint);
      ...
      if (innerBottom > innerTop) {
        LinearGradient vertical = new LinearGradient(left, innerTop, left + shadowX, innerTop,
            new int[] {0x00000000, 0x44000000, 0x00000000}, new float[] {0f, 0.5f, 1f},
            Shader.TileMode.CLAMP);

        shadowPaint.setShader(vertical);

        canvas.drawRect(left - shadowX, innerTop, left, innerBottom, shadowPaint);

        canvas.drawRect(right, innerTop, right + shadowX, innerBottom, shadowPaint);
    ...
    private void buildCornerPath(float radius) {
      ...
      float shadow = Math.max(1f, shadowSize);

      RectF outer = new RectF(-radius, -radius, radius, radius);

      RectF inner = new RectF(-radius + shadow, -radius + shadow, radius - shadow, radius - shadow);

      cornerPath.setFillType(Path.FillType.EVEN_ODD);

      cornerPath.addRoundRect(outer, radius, radius, Path.Direction.CW);

      if (inner.width() > 0 && inner.height() > 0) {
        cornerPath.addRoundRect(
            inner, Math.max(0, radius - shadow), Math.max(0, radius - shadow), Path.Direction.CCW);
      }

      float gradientRadius = radius + shadow;

      cornerShadowPaint.setShader(
          new RadialGradient(0, 0, gradientRadius, new int[] {0x55000000, 0x33000000, 0x00000000},
              new float[] {0f, radius / Math.max(gradientRadius, 1f), 1f}, Shader.TileMode.CLAMP));
    }
    ...
    float getMinWidth() {
      float horizontal = maxShadowSize * 2f;

      if (addPaddingForCorners) {
        horizontal += (1f - (float) COS_45) * cornerRadius * 2f;
      }
    ...
    float getMinHeight() {
      float vertical = maxShadowSize * SHADOW_MULTIPLIER * 2f;

      if (addPaddingForCorners) {
        vertical += (1f - (float) COS_45) * cornerRadius * 2f;
      }
    ...
    void getMaxShadowAndCornerPadding(Rect out) {
      float h = calculateHorizontalPadding(maxShadowSize, cornerRadius, addPaddingForCorners);

      float v = calculateVerticalPadding(maxShadowSize, cornerRadius, addPaddingForCorners);

      out.set((int) Math.ceil(h), (int) Math.ceil(v), (int) Math.ceil(h), (int) Math.ceil(v));
    }

    static float calculateHorizontalPadding(float shadowSize, float radius, boolean addPaddingForCorners) {
      if (addPaddingForCorners) {
        return shadowSize + (1f - (float) COS_45) * radius;
      }

      return shadowSize;
    }

    static float calculateVerticalPadding(float shadowSize, float radius, boolean addPaddingForCorners) {
      if (addPaddingForCorners) {
        return shadowSize * SHADOW_MULTIPLIER + (1f - (float) COS_45) * radius;
      }

      return shadowSize * SHADOW_MULTIPLIER;
    }
    ...
    private void updateTintFilter() {
      ...
      int c = tint.getColorForState(getState(), Color.TRANSPARENT);

      tintFilter = new PorterDuffColorFilter(c, tintMode);
    ...
    public boolean isStateful() {
      return color != null && color.isStateful() || tint != null && tint.isStateful();
    }
    ...
    protected boolean onStateChange(int[] state) {
      ...
      if (color != null) {
        int newColor = color.getColorForState(state, color.getDefaultColor());
