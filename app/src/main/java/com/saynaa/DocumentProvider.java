...
public class DocumentProvider extends DocumentsProvider {
  ...
  // These will be set dynamically in onCreate()
  private String PACKAGE_NAME;
  private File BASE_DIR;
  private int APP_NAME; // String resource ID
  private int APP_ICON; // Drawable resource ID

  private static final String[] DEFAULT_ROOT_PROJECTION = new String[] {Root.COLUMN_ROOT_ID,
      Root.COLUMN_MIME_TYPES, Root.COLUMN_FLAGS, Root.COLUMN_ICON, Root.COLUMN_TITLE,
  ...
  public boolean onCreate() {
    ...
    // App data dir: /data/data/<package>
    BASE_DIR = getContext().getFilesDir().getParentFile();

    // Get app name and icon resource IDs dynamically
    ApplicationInfo appInfo = getContext().getApplicationInfo();
    APP_NAME = appInfo.labelRes;
  ...
  public Cursor queryRoots(String[] projection) {
    ...
    final String applicationName = getContext().getString(APP_NAME);
  ...
  public String renameDocument(String documentId, String displayName) throws FileNotFoundException {
    ...
    if (newFile.exists()) {
      throw new FileNotFoundException("File with name " + displayName + " already exists");
    }

    boolean success = file.renameTo(newFile);
    if (!success) {
      throw new FileNotFoundException("Failed to rename " + file.getAbsolutePath());
    }
