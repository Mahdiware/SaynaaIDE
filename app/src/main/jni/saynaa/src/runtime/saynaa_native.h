...
typedef int (*nextSlot_t)(VM*, bool);
typedef int (*allocSlot_t)(VM*, uint32_t);
typedef void (*freeSlot_t)(VM*, uint32_t, uint32_t);
...
typedef struct {
  ...
  nextSlot_t nextSlot_ptr;
  allocSlot_t allocSlot_ptr;
  freeSlot_t freeSlot_ptr;
...
NativeApi MakeNativeAPI() {
  ...
  api.nextSlot_ptr = nextSlot;
  api.allocSlot_ptr = allocSlot;
  api.freeSlot_ptr = freeSlot;
